...
float turn_on_robot::hcsr04_Trans(uint8_t* Data_)
{
  float distance_return=0;
  unsigned int temp=0;
 for(int i=0;i<4;i++)
     temp |= Data_[i]<<(24-8*i);
  distance_return = (float)temp/100000000;
  return distance_return;  // m
}
/**************************************
Date: June 29, 2020
...
// publish hcsr04 distance  unit:m
void turn_on_robot::Publish_hcsr04_Dis()
{
    ...
    //double intensities[num_readings];
        ...
        scan.header.frame_id = sonar_frame_id;
        ...
        scan.time_increment = (1 / sonar_laser_frequency) / (num_readings);
        ...
        {       
                scan.ranges[i] = hc_sr04_distance_f;
        }
        ROS_INFO("hc_sr04_distance_f:%f\n m",hc_sr04_distance_f);
        hcsr04_scan_publisher.publish(scan);
}
/**************************************
Date: May 31, 2020
Function: 发布里程计相关信息
 ...
 if (Receive_Data.Frame_Header == FRAME_HEADER )//判断帧头
  {
    ...
    { 
      if (Receive_Data.rx[22+4] == Check_Sum(22+4,READ_DATA_CHECK))//校验位检测
      {
        ...
        uint8_t hcsr04_data[4];
        for(int i=0;i<4;i++) hcsr04_data[i]=Receive_Data.rx[i+22];
        hc_sr04_distance_f = hcsr04_Trans(hcsr04_data);
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  /*************************************new***********/
private_nh.param<std::string>("sonar_frame_id", sonar_frame_id, "sonar_laser");
private_nh.param<float>("angle_min", angle_min, 135.0f);
private_nh.param<float>("angle_max", angle_max, 225.0f);
private_nh.param<int>("num_readings", num_readings, 20);
private_nh.param<double>("sonar_laser_frequency", sonar_laser_frequency, 10);
private_nh.param<float>("rangemin", rangemin, 0.15);
private_nh.param<float>("rangemax", rangemax, 10);
  ...
  //new
  hcsr04_scan_publisher = n.advertise<sensor_msgs::LaserScan>("/sonar_scan_data", 1000);
